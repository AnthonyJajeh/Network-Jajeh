...
parfor vf_index = 1:length(vf0_values)
    ...
    w_v = A_sv_current;      % conductance-weighted
    w_h = A_sh_current;
    d_v = w_v / sum(w_v(:));           % global normalization
    d_h = w_h / sum(w_h(:));
    ...
    for iter = 2:max_iters
        % incremental EPS concentration this iteration (mg/L → kg/m^3)
...
[eff_perm, sv, sh, A_sv_new, A_sh_new] = run_network_model( ...
    ...
    A_sv_current, A_sh_current, [], [], h, V_pore, fixed_clog);
        ...
        % Diagnostics
        cumEPS_conc_kgm3 = EPS_conc_vec(iter)*1e-3;  % current total conc
        fprintf('vf0 %.3f | iter %2d | dEPS = %.3e kg/m^3 | cumEPS = %.3e kg/m^3 | p = %.4e m^2 | p/p0 = %.3f\n', ...
    vf0, iter, dEPS_conc_kgm3, cumEPS_conc_kgm3, eff_perm, eff_perm/perm_0);

        % Sanity: area should not increase
        total_area_old = sum(A_sv_current(:)) + sum(A_sh_current(:));
        total_area_new = sum(A_sv_new(:))     + sum(A_sh_new(:));
        if total_area_new > total_area_old + 1e-12
            warning('Total area increased (%.6e -> %.6e).', total_area_old, total_area_new);
        end
        ...
        % Rebuild weights from current geometry (global normalization)
        w_v = A_sv_current;
        w_h = A_sh_current;
        d_v = w_v / sum(w_v(:));
        d_h = w_h / sum(w_h(:));

        % Update cumulative EPS mass/volume accounting
        dEPS_mass   = dEPS_conc_kgm3 * V_pore;        % kg
        dEPS_volume = dEPS_mass / rho_EPS;                 % m^3
        eps_mass_total_vec(iter)   = eps_mass_total_vec(iter-1)   + dEPS_mass;
        eps_volume_total_vec(iter) = eps_volume_total_vec(iter-1) + dEPS_volume;

        % Advance ODE with current geometry-induced permeability
        tspan = [0 5];
        EPS_conc_mgL_current = EPS_conc_vec(iter);
        IC = [N_vec(iter); A_vec(iter); EPS_conc_mgL_current];
        ode_fun = @(t,D) NAE(t, D, perm_0, eff_perm, phi, psi, nu_1, nu_2, xi, delta, eta, gamma);
        % Non-negativity guard is helpful
